...
void handleHttpRequest(){
  String response = "";
  for (int i = 0; i < 6; i++){
    response += "Z&auml;hler " + String(i+1) + ": " + String(impulse[i]) + "\n";
  }
...
void sendMqttMessage(){ 
  ...
  for (int i = 0; i < 6; i++) {
    ...
    String topic = "/ESP8266/EnergyLink/Zaehler_" + String(i) + "/Impulse";
    client.publish(const_cast<char*>(topic.c_str()), buffer);
  }
  for (int i = 0; i < 6; i++) {
    char buffer[10];
    sprintf(buffer, "%lu", entprellZeit[i]);
    String topic = "/ESP8266/EnergyLink/Zaehler_" + String(i) + "/Entprellzeit";
    client.publish(const_cast<char*>(topic.c_str()), buffer);
  }
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
      ...
      client.subscribe("/ESP8266/EnergyLink/*/Entprellzeit");
    } else {
...
void setup(){
  ...
  WiFi.hostname("ESP8266_EnergyLink");
